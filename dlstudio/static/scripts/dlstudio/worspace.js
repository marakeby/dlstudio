...
var workspace_data_dic = []; // create an empty array
function add_cell_to_workspace(cell, data) {
    workspace_data_dic[cell.id]= data
    graph.addCells([cell])
}

paper.on('cell:pointerclick', function (cellView, evt, x, y) {
    id = cellView.model.id
    data = workspace_data_dic[id]
    console.log('I was clicked, dare u??' + id + data)
    //console.log(data)
    populate_prop_editor(id, data)
})

function update_data_entery(id, data) {
    console.log('update entry called'+ id + data)
    workspace_data_dic[id]= data
}